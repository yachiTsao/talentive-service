...
- TypeScript 5.5 / Node.js 18+ + Express 4.19（現有）、Node.js 內建 `fs`（現有）；**無新增執行期依賴** (004-chart-aggregation-api)
- `jobs.json`（唯讀，路徑由 `process.env.OUTPUT` 決定，預設 `/app/data/jobs.json`） (004-chart-aggregation-api)
...
## Recent Changes
- 004-chart-aggregation-api: Added TypeScript 5.5 / Node.js 18+ + Express 4.19（現有）、Node.js 內建 `fs`（現有）；**無新增執行期依賴**
